...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not answer_tokens:
            return 1.0
        context_tokens = _tokenize(context if context else "")
    ...
    def evaluate_context_precision(
        ...
        for k, is_rel in enumerate(relevant_flags, start=1):
            if is_rel:
                running_relevant += 1
                precisions.append((running_relevant / k) * 1.0)
            else:
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric: {rubric}\n"
            "Please evaluate the answer according to the rubric and return scores as JSON."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if item_scores:
                vals = [float(v) for v in item_scores.values()]
        ...
        positional_bias = False
        if len(item_avgs) >= 2:
            first_avg = item_avgs[0]
            rest_avg = sum(item_avgs[1:]) / len(item_avgs[1:])
            if first_avg > rest_avg + 0.15:
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_val = scores[0][1]
        lowest_metrics = [name for name, val in scores if abs(val - min_val) < 1e-6]
        if len(lowest_metrics) > 1 and min_val < 0.5:
    ...
    def generate_improvement_suggestions(
        ...
        if cat_counts.get("irrelevant", 0) > 0:
            suggestions.append("Refine prompt clarity and system instructions to focus answer on the user question.")
        if cat_counts.get("incomplete", 0) > 0:
            suggestions.append("Increase context window and top-k retrieval limit to include comprehensive details.")
        if cat_counts.get("off_topic", 0) > 0:
            suggestions.append("Add intent classification and routing guardrails before response generation.")
        default_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Fine-tune system prompt to restrict model from guessing out-of-context facts"
        ]
